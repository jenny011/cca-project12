...
class Controller():
    ...
    def new_periodic_scheduler(self):
        ...
        # run ferret
        self.scheduler.start_one(0, True)
        print("<===============Ferret===============>")
        ...
        while True:
            ...
            if mc_prc_cpu_util <= 50:
                self.scheduler.unpause_one()
            else:
                self.scheduler.pause_one()

            if mc_prc_cpu_util <= 80:
            ...
            collocate_one_finished = self.scheduler.is_finished(0, True)
            if collocate_one_finished:
                self.scheduler.start_one(0, True)
            if self.scheduler.is_finished(0):
                break
            time.sleep(0.5)

        # run freqmine and blackscholes
        print("<===============Freqmine===============>")
        print("<===============Blackscholes===============>")
        ...
        # unpause
        self.scheduler.unpause_one()
        self.scheduler.update_one(0, "2,3", True)
        while True:
            collocate_one_finished = self.scheduler.is_finished(0, True)
            if collocate_one_finished:
