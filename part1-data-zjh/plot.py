...
def plotline(x, y, xerr, yerr, labelname):
    plt.errorbar(x, y, xerr=xerr, yerr=yerr,fmt=plotstyle, linewidth=1, markersize=6,marker = m, markerfacecolor='none',markeredgewidth=1,capsize=3,capthick=1, label=labelname)
...
fig = plt.figure()
plotline(rawx_avg, rawy_avg, rawx_std, rawy_std, "no interference")
...
plt.title("Part 1: Performance of Memcached with Interference")
plt.xlabel("QPS [queries per second]")
plt.ylabel("95th percentile latency [ms]")

plt.xticks([x for x in range(0, 85000, 5000)], [f"{x}k" for x in range(0, 85, 5)])
plt.yticks([y for y in range(0, 11, 1)])

plt.legend(title='interference', loc='upper left')
ax = plt.gca()
